...
public final class WireAsset extends BaseAsset implements WireEmitter, WireReceiver {
        ...
        for (final Entry<String, Channel> channelEntry : channels.entrySet()) {
            ...
            if (wireRecordProperties.containsKey(channelName)) {
                ...
                if (channel.getValueType() == value.getType()) {
                    assetRecordsToWriteChannels.add(channel.createWriteRecord(value));
                }
            }
        }
    ...
    private void logErrorMessage(final ChannelStatus assetStatus) {
    ...
    private void writeChannels(final List<ChannelRecord> assetRecordsToWriteChannels) {
        requireNonNull(assetRecordsToWriteChannels, message.channelRecordsNonNull());
